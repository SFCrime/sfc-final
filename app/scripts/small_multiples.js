...
(function() {
    ...
    SmallMultiples = function() {
        var area, bisect, caption, chart, circle, curHour, data, format, height, line, margin, mousemove, mouseout, mouseover, setupScales, width, xScale, xValue, yAxis, yScale, yValue;
        width = 150;
        height = 120;
        margin = {
            top: 15,
        ...
        chart = function(selection) {
            return selection.each(function(rawData) {
                ...
                lines.append("text").attr("class", "static_hour").attr("text-anchor", "start").style("pointer-events", "none").attr("dy", 13).attr("y", height).attr("x", 0).text(function(c) {
                    return xValue(c.values[0]).getHours();
                });
                lines.append("text").attr("class", "static_hour").attr("text-anchor", "end").style("pointer-events", "none").attr("dy", 13).attr("y", height).attr("x", width).text(function(c) {
                    return xValue(c.values[c.values.length - 1]).getHours();
                });
                ...
                curHour = lines.append("text").attr("class", "hour").attr("text-anchor", "middle").style("pointer-events", "none").attr("dy", 13).attr("y", height);
                return g.append("g").attr("class", "y axis").call(yAxis);
            });
        };
        mouseover = function() {
            circle.attr("opacity", 1.0);
            d3.selectAll(".static_hour").classed("hidden", true);
            return mousemove.call(this);
        };
        mousemove = function() {
            var date, index, hour;
            hour = xScale.invert(d3.mouse(this)[0]).getHours();
            ...
            return curHour.attr("x", xScale(date)).text(hour);
        };
        mouseout = function() {
            d3.selectAll(".static_hour").classed("hidden", false);
    ...
    $(function() {
        ...
        queue().defer(d3.csv, "data/small_mult.csv").await(display);
        return d3.select("#button-wrap").selectAll("button").on("click", function() {
            var id;
            id = d3.select(this).attr("id");
            d3.select("#button-wrap").selectAll("button").classed("btn-warning", false);
            d3.select("#" + id).classed("btn-warning", true);
            return $("#small-mult").isotope({
                sortBy: id
            });
        });
